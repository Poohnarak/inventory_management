...
async function getById(req, res, next) {
  try {
    const ingredient = await ingredientService.getById(parseInt(req.params.id, 10));
...
async function remove(req, res, next) {
  try {
    await ingredientService.remove(parseInt(req.params.id, 10));
...
async function getLowStock(req, res, next) {
  try {
    const ingredients = await ingredientService.getLowStockRaw();
